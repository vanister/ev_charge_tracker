...
- [ ] 1. Add `MaintenanceRecordSchema` and `MAINTENANCE_TYPES` to `src/data/schemas.ts` [§2 — MaintenanceType Enum, MaintenanceRecord Schema]
  - Add the `MAINTENANCE_TYPES` const array (10 values: `tire_rotation` … `other`)
  - Add `MaintenanceType` derived type
  - Add `MaintenanceRecordSchema` Zod schema with all fields from §2 (id, vehicleId, type, description, costCents?, mileage?, serviceProvider?, servicedAt, nextDueDate?, nextDueMileage?, notes?, createdAt)
...
- [ ] 3. Add `maintenanceRecords` table to Dexie schema in `src/data/db.ts` [§2 — Dexie Table]
